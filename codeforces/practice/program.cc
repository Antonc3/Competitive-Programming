...
int main(){
	...
	while(t--){
		...
		for(int i = 0; i < m; i++){
			...
			if(r == n){
				cout <<lar[l-1]-smal[l-1]+1<<endl;
				continue; 
			}
			if(l == 1){
				cout << larBack[r-1]-smalBack[r-1]+1 << endl;
				continue;
			}
			int lSmall = smal[l-1];
			int lLar = lar[l-1];
			int rSmall = min(0,pref[l-1]+smalBack[r-1]);
			int rLar = max(0,larBack[r-1]+pref[l-1]);
			int ans = lLar+rLar-lSmall-rSmall+1;
			cout << ans << endl;
		}
	}
}