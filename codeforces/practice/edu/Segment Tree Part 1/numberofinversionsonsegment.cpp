...
struct segtree{
    ...
    item merge(item a, item b){
        item res = item();
        res.invCnt = a.invCnt+b.invCnt;
        for(int i = 1; i < MAXNUM; i++){
            int agreater = a.cnt[MAXNUM-1]-a.cnt[i];
            //cout << "agreater: " << agreater<< " I: " << i <<endl;
            res.invCnt += 1ll*(b.cnt[i]-b.cnt[i-1])*agreater;
        }
